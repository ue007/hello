...
(async () => {
  ...
  await deal("./in", "./out", 3000, true, "cuda"); // none, qsv, cuda
  if (timer) {
    clearInterval(timer);
  }
  saveFile();
  ...
  if (hardType && !hardTypeSet.has(hardType)) {
    throw new Error(
      "不支持的加速方案:" +
        hardType +
        ",仅支持:空," +
        JSON.stringify(hardTypeSet)
    );
  }
  let fileList = fs.readdirSync(inputPath);
  for (let i in fileList) {
    let name = fileList[i];
    let filePath = path.join(inputPath, name);
    ...
    let res = JSON.parse(
      cmd.exec(
        `ffprobe.exe "${filePath}" -show_streams -select_streams v -show_format  -print_format json`,
        { encoding: "utf-8" }
      ).stdout
    );
    ...
    let isH264 =
      res.streams.filter((item) => item.codec_name === "h264").length > 0;
    if (!isH264) {
    ...
    let is10Bit =
      res.streams.filter((item) => item.bits_per_raw_sample === "10").length >
      0;
    ...
    let newFilePath = path.join(outPath, newName);
    let cmdStr = `ffmpeg.exe ${hwType} ${
      is10Bit ? "" : decodeType
    } -i "${filePath}" -map 0 ${encodeType} -maxrate ${bitRate}K -c:a copy -c:s copy -y "${newFilePath}"`;
    console.log(cmdStr);
    ...
    if (
      ...
      fs.statSync(newFilePath).size <= 10000 ||
      cmdRes.stderr.indexOf("Error") > -1
    ) {
    ...
    if (changeName) {
      ...
      fileList.forEach((item) => {
        if (item.startsWith(namePart1) && item != name) {
          ...
          if (item != temp) {
            fs.renameSync(path.join(outPath, item), path.join(outPath, temp));
          }
        }
      });
    }

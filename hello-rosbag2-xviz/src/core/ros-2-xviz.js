...
export class ROS2XVIZConverter {
	...
	_makeConvertersForTopic(entry, topicMessageTypes, aux) {
		...
		if (converter) {
			...
			console.log('ConverterClass:', ConverterClass);
			if (!ConverterClass) {
				this.verbose(`ROS2XVIZConverter cannot find the converter with name '${converter}' for topic '${topic}'`);
			}
		} else {
			...
			if (!msgType) {
				...
				if (!ConverterClass) {
					this.verbose(`ROS2XVIZConverter cannot find the converter for message type '${msgType}' for topic '${topic}'`);
				}
			}
		}
	...
	initializeConverters(topicMessageTypes, aux) {
		...
		if (count > 0) {
			...
			for (const key in topicMessageTypes) {
				this._makeConvertersForTopic({ topic: key, type: topicMessageTypes[key] }, topicMessageTypes, aux);
			}
		}
	...
	async buildMessage(frames) {
		const xvizBuilder = new XVIZBuilder(this.metadata, this.disableStreams, {});

		for (const instance of this.instances) {
			await instance.convertMessage(frames, xvizBuilder);
		}
